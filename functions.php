...
function show_custom_logo( $size = 'medium' ) {
	if ( $custom_logo_id = get_theme_mod( 'custom_logo' ) ) {
		...
		$logo_url = get_stylesheet_directory_uri() . '/app/css/images/custom-logo.png';
		$w        = 200;
		$h        = 160;
...
function foundation_scripts_and_styles() {
	if ( ! is_admin() ) {
		...
		//system
		wp_enqueue_style( 'custom', get_template_directory_uri() . '/app/dist/app.css', null, null );/*2rd priority*/
		wp_enqueue_style( 'style', get_template_directory_uri() . '/style.css', null, null );/*1st priority*/


		//custom javascript
		wp_enqueue_script( 'app', get_template_directory_uri() . '/app/dist/app.js', null, null, true ); /* This should go first */
...
add_filter( 'acf/load_field/type=google_map', function ( $field ) {
	...
	$api_args       = array(
		'key' => get_theme_mod( 'google_maps_api' ) ?: '',
		'language' => 'en',
		'v' => '3.exp'
	);
...
function register_google_maps_settings( $wp_customize ) {
	...
	$wp_customize->add_setting( 'google_maps_api', array(
		'default' => 'AIzaSyBgg23TIs_tBSpNQa8RC0b7fuV4SO', //default key
	) );
